...
async function synthPass(payload,M,ls){
  var sys='Ты — Viora AI, главный продюсер И строгий шеф-редактор («Критик») в одном лице. Сделай ФИНАЛЬНЫЙ СИНТЕЗ разбора канала: главная утечка роста (1-2 предложения с цифрами), скор и разбивка, конкретные изменения и план по неделям. Требования критика: ноль воды и общих фраз; в каждом пункте — конкретная цифра канала и готовый пример (переписанный заголовок / идея превью / фраза хука); ничего не выдумывай сверх данных. ФОРМАТ СОВЕТА: ЧТО изменить -> ПОЧЕМУ (цифра) -> ЭФФЕКТ -> ПРИМЕР. '+_AUD_RULE+' Верни СТРОГО валидный JSON, без markdown.\n'+_kbp(['visp','hunt','funnel','scenarioErrors','mission','comments']);
  var schema='Схема ответа: {"main_leak":"1-2 предложения: главный ограничитель роста, с цифрами","leak_tag":"короткий тег, 2-4 слова","score":"число 0-100","score_breakdown":[{"factor":"Вовлечённость","value":0},{"factor":"Регулярность","value":0},{"factor":"Доля хитов","value":0},{"factor":"Упаковка","value":0}],"concrete_changes":[{"change":"действие без воды","target":"ролик/рубрика","effect":"эффект в цифрах","priority":"high|medium|low"}],"action_plan":[{"step":"задача с деталями","why":"эффект в цифрах","priority":"high|medium|low","week":1}]}';
